        ...
        <v-card outlined>
            <v-list flat subheader>
                ...
                <v-list-item v-for="(item, index) in value" :key="index">
                    <v-list-item-content>
                        <v-list-item-title>{{ getItemText(item) }}: {{ item[insertKey] }}</v-list-item-title>
                    </v-list-item-content>
                    <v-list-item-action>
                        <div class="actions">
...
export default {
    ...
    methods: {
        ...
        async getItems(search) {
            ...
            try {
                const data = await this.$store.dispatch(`${this.model}/nomenclature`, params)
                this.items = data
            } catch (e) {
                this.pushAlert({
                    type: 'error',
        ...
        onAdd() {
            this.$emit('input', [...this.value, { ...this.currentItem, [this.insertKey]: this.currentValue }])
