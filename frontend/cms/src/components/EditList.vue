    ...
    <div>
        <v-card outlined>
            <v-list flat :subheader="Boolean(title.length)">
                ...
                <v-list-item>
                    <v-list-item-content>
                        <v-list-item-title><v-btn color="primary" @click="onCreate">Добавить</v-btn></v-list-item-title>
                    </v-list-item-content>
                </v-list-item>

                <v-list-item v-for="(item, index) in sortedValue" :key="item.id">
                    <v-list-item-content>
                        <v-list-item-title class="block-header" @click="openItemDetail(item)">{{
                            getItemText(item)
                        }}</v-list-item-title>
                    </v-list-item-content>
                    <v-list-item-action>
                        <div class="actions">
                            ...
                            ></picker>
                            <v-btn icon @click.stop="decrementOrder(item, index)">
                                <v-icon>mdi-arrow-down-circle</v-icon>
                            </v-btn>
                            <v-btn icon @click.stop="incrementOrder(item)">
        ...
        <v-dialog transition="dialog-top-transition" :value="showDetailModal" @input="closeDetailModal">
            <template>
                <v-card outlined class="modal-detail">
                    <v-toolbar color="primary" dark
                        >Добавить блок<v-spacer></v-spacer
                        ><v-btn icon @click="closeDetailModal"> <v-icon>mdi-close</v-icon></v-btn></v-toolbar
                    >
                    <v-card-text>
                        <picker
                            :multiple="!single"
                    ...
                    <v-card-actions>
                        <v-btn depressed color="primary" @click="confrimEntities">Сохранить</v-btn></v-card-actions
                    >
                </v-card>
            </template>
        </v-dialog>
...
export default {
    ...
    methods: {
        ...
        getItemText(item) {
            return typeof this.itemText === 'function' ? this.itemText(item) : item[this.itemText]
        },
        openDetailModal() {
            this.showDetailModal = true
        },
